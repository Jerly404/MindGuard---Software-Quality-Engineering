...
class EvaluationRepository:
    ...
    async def list_by_user_asc(self, user_id: int) -> List[Evaluacion]:
        result = await self.db.execute(
            select(Evaluacion).where(Evaluacion.id_usuario == user_id).order_by(Evaluacion.fecha.asc())
        )
        return list(result.scalars().all())

    async def list_by_user_desc(self, user_id: int) -> List[Evaluacion]:
        result = await self.db.execute(
            select(Evaluacion).where(Evaluacion.id_usuario == user_id).order_by(Evaluacion.fecha.desc())
        )
        return list(result.scalars().all())
