...
from app.models.base import Usuario, Evaluacion, AsignacionProfesional, Cita, TransaccionMock
...
from app.schemas.evaluation import Evaluation, DetailedAIAnalysis
...
class EvaluationService:
    ...
    async def create_chat_evaluation(self, user_id: int, messages: List[Dict[str, str]]) -> Evaluation:
        ...
        es_alto = "alto" in ansiedad_label.lower() or "grave" in ansiedad_label.lower() or "alto" in depresion_label.lower() or "grave" in depresion_label.lower()
        ...
        ansiedad_val = 0.8 if "alto" in ansiedad_label.lower() or "grave" in ansiedad_label.lower() else (0.5 if "medio" in ansiedad_label.lower() or "moderado" in ansiedad_label.lower() else 0.2)
        depresion_val = 0.8 if "alto" in depresion_label.lower() or "grave" in depresion_label.lower() else (0.5 if "medio" in depresion_label.lower() or "moderado" in depresion_label.lower() else 0.2)
...
class AssignmentService:
    ...
    async def simulate_payment_and_assign(self, patient_id: int, professional_id: int, monto: float, metodo: str) -> Dict[str, Any]:
...
class AppointmentService:
    ...
    async def create_appointment(self, professional_id: int, patient_id: int, fecha_cita: datetime, mensaje_seguimiento: Optional[str] = None) -> Cita:
        # Verificar asignación activa
        asignacion = await self.asig_repo.get_active_by_patient(patient_id)
        if not asignacion:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="El paciente no tiene una suscripción premium activa"
            )
