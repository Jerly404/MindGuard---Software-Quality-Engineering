...
const AdminDashboard: React.FC = () => {
  ...
  const filteredUsers = allUsers.filter((u) => {
    ...
    if (activeTab === "pacientes")
      return matchesSearch && (role === "usuario" || role === "paciente");
    if (activeTab === "profesionales")
      return matchesSearch && role === "profesional";
    return matchesSearch;
  });

  const stats = {
    total: allUsers.length,
    pacientes: allUsers.filter((u) =>
      ["usuario", "paciente"].includes((u.rol || "").toLowerCase()),
    ).length,
    profesionales: allUsers.filter(
      (u) => u.rol?.toLowerCase() === "profesional",
    ).length,
  };
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {loading ? (
            ...
            filteredUsers.map((user) => (
              ...
              >
                <div
                  className={`absolute top-0 right-0 h-24 w-24 -mr-8 -mt-8 rounded-full opacity-5 ${
                ...
                <div className="flex items-start gap-5">
                  <div
                    className={`h-16 w-16 min-w-[64px] rounded-2xl flex items-center justify-center text-2xl font-black shadow-inner ${
                      user.rol?.toLowerCase() === "profesional"
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="font-bold text-slate-800 text-lg truncate max-w-[150px]">
                        {user.nombre}
                      </h3>
                      <span
                    ...
                    {user.rol?.toLowerCase() === "profesional" && (
                      <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 mb-4">
                        ...
                        <p className="text-xs font-bold text-slate-700">
                          {user.especialidad || "Psicoterapeuta"}
                        </p>
                        <p className="text-[10px] text-indigo-600 font-medium mt-1">
                          Col. {user.colegiatura || "98765-P"}
                        </p>
                      </div>
                    )}
          ...
          <div className="bg-white w-full max-w-xl rounded-[3rem] shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10">
            <div className="bg-indigo-600 p-10 text-white relative">
              ...
              <p className="text-indigo-100 mt-2">
                Agrega un nuevo especialista a la red médica de MindGuard.
              </p>
              <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    value={newPro.nombre}
                    onChange={(e) =>
                      setNewPro({ ...newPro, nombre: e.target.value })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <input
                    type="email"
                    required
                    placeholder="julian@mindguard.ai"
                    className="w-full px-6 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-600 transition-all text-sm"
                    value={newPro.email}
                    onChange={(e) =>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewPro({ ...newPro, colegiatura: e.target.value })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewPro({ ...newPro, especialidad: e.target.value })
                    }
                  >
